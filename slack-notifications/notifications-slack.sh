...
## Ubuntu 20.04 n 22.04
sudo sed -i 's/:t>0/:t<0/g' ~/tmp/slack/dist/main.*.js
## Ubuntu 24.04
sudo sed -i 's/:t>0/:t<0/g' ~/tmp/slack/dist/main.*.cjs
# Turn off red dot notifications (only if you want)
## Ubuntu 20.04 n 22.04
echo "mod f Ubuntu 20.04 n 22.04. Ignore errors if not these OS"
sudo sed -i 's/return i>0/return i<0/g' ~/tmp/slack/dist/main.*.js
sudo sed -i 's/return n>0/return n<0/g' ~/tmp/slack/dist/main.*.js
## Ubuntu 24.04
echo "mod f Ubuntu 24.04. Ignore errors if not this OS"
sudo sed -i 's/return i>0/return i<0/g' ~/tmp/slack/dist/main.*.cjs
sudo sed -i 's/return n>0/return n<0/g' ~/tmp/slack/dist/main.*.cjs
